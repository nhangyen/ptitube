...
def parse_args():
    ...
    parser.add_argument('--sample_ratio', type=float, default=1.0, help='Tỷ lệ phần trăm dữ liệu muốn train (ví dụ 0.1 là 10% data)')
...
def load_and_preprocess_data(args):
    ...
    if hasattr(args, 'sample_ratio') and args.sample_ratio < 1.0:
        train_size = int(len(train_feats_selected) * args.sample_ratio)
        test_size = int(len(test_feats_selected) * args.sample_ratio)
        
        # Chọn random để tránh bị hụt tập người dùng
        np.random.seed(args.seed)
        train_idx = np.random.choice(len(train_feats_selected), train_size, replace=False)
        test_idx = np.random.choice(len(test_feats_selected), test_size, replace=False)
        
        train_feats_selected = train_feats_selected[train_idx]
        test_feats_selected = test_feats_selected[test_idx]
        print(f"📌 Đã bốc ngẫu nhiên {args.sample_ratio*100}% dữ liệu: Train={train_size} mẫu, Test={test_size} mẫu")
