...
public class ModerationService {
    ...
    @Transactional
    public void approveVideo(UUID queueId, UUID adminId, String notes) {
        ModerationQueue queue = queueRepository.findById(queueId)
                .orElseThrow(() -> new RuntimeException("Queue item not found"));
        User admin = userRepository.findById(adminId)
                .orElseThrow(() -> new RuntimeException("User not found"));
        ...
        notificationService.createModerationNotification(
                video.getUser().getId(), video.getId(), NotificationType.video_approved);
    }
    ...
    public void rejectVideo(UUID queueId, UUID adminId, String reason) {
        ...
        notificationService.createModerationNotification(
                video.getUser().getId(), video.getId(), NotificationType.video_rejected);
    }
    ...
    private ModerationQueueResponse toQueueResponse(ModerationQueue queue) {
        ...
        resp.setVideoUrl(queue.getVideo().getStreamUrl());
